...
public class DonateToast implements Toast {
    ...
    public Visibility draw(MatrixStack matrices, ToastManager manager, long startTime) {
        ...
        manager.getClient().textRenderer.draw(matrices, Text.literal("Wish to support Illuminations?"), 34, 7, -256);
        manager.getClient().textRenderer.draw(matrices, Text.literal("Get cool cosmetics for only 5€!"), 34, 18, -1);
        manager.getClient().textRenderer.draw(matrices, Text.literal("More info: doctor4t.uuid.gg/donators").copy().fillStyle(Style.EMPTY.withColor(Formatting.GREEN)), 34, 29, -1);
